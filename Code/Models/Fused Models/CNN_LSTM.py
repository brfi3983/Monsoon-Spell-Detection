...
from keras.layers import Dense, Conv2D, Dropout, MaxPooling2D, Flatten, Input, concatenate, BatchNormalization, LSTM, CuDNNLSTM
...
w_array[0, 2] = 1.5   #penalty for classifying a dry day as wet
w_array[2, 1] = 1.2   #penalty for classifying a wet day as normal
w_array[2, 0] = 1.5   #penalty for classifying a wet day as dry
...
def plotModelAccuracy(history, filename):
	...
	plt.savefig(path + 'results/FusedCNN_LSTM/' + filename + '.png')
	plt.close()
...
def modelPerformanceClassificationMetrics(y_test_nor, y_pred_nor, filename):
	file = open(path + 'results/FusedCNN_LSTM/' + filename + ".txt", 'w')
...
def CombinedModel(Cnn_model, lstm_model, x_train_2d, y_train_2d, x_train_1d, x_valid_2d, x_valid_1d, y_valid_2d, lr, bs, epochs):
    ...
    rms = RMSprop(lr = lr, decay=1e-6)
    model.compile(loss=defined_loss_func, optimizer=rms, metrics=['accuracy'])
...
# =====================================================================================================
# def CombinedModel(Cnn_model, LSTM_model, x_train, y_train, x_train_rain, x_valid, x_valid_rain, y_valid, lr, bs):
...
# 	dense2 = Dense(32, activation='relu')(drop1)
# 	output = Dense(3, activation='softmax')(dense2)
# 	# print('output and training shape', output.shape, y_train.shape)
# 	# exit()
# 	model = Model(inputs=[Cnn_model.input, Lstm_model.input], outputs=output)

# 	opt = Adam(lr=lr)
# 	model.compile(
# 		optimizer=opt, loss=CustomLoss, metrics=['accuracy'])
# 	# model.compile(
# 	# 	optimizer=opt, loss='categorical_crossentropy', metrics=['accuracy'])

# 	model.summary()
# 	# print(x_train.shape, x_train_rain.shape)
# 	history = model.fit([x_train, x_train_rain], y_train, batch_size=bs,  epochs=3, validation_data=([
...
def main():
	...
	# Balancing Data
	x_train_1d, y_train_1d = SMOTE().fit_resample(x_train_1d, y_train_1d)
	x_valid_1d, y_valid_1d = SMOTE().fit_resample(x_valid_1d, y_valid_1d)
	x_test_1d, y_test_1d = SMOTE().fit_resample(x_test_1d, y_test_1d)
	...
	select = 2
	[x_train_2d, x_valid_2d, x_test_2d, y_train_2d, y_valid_2d, y_test_2d] = balanceClassesByOversampling(x_train_2d, x_valid_2d, x_test_2d, y_train_2d, y_valid_2d, y_test_2d, select)
	...
	batch_size = [50]  # , 50, 75, 100, 150, 200]
	learn_rate = [0.0001]  # , 0.0001]
	for bs in batch_size:
		for lr in learn_rate:

			# CNN model
			cnn_model = CNN_model(x_train_2d)

			# LSTM model
			print('1d shape',x_train_1d.shape)
			# x_train_1d = np.reshape(x_train_1d, (x_train_1d.shape[0], x_train_1d.shape[1]))
			# x_train_1d = np.swapaxes(x_train_1d, 0, -1)
			# x_train_1d = np.swapaxes(x_train_1d, 1, 2)
			# print('1d shape reshaped',x_train_1d.shape)
			lstm_model = LSTM_model(x_train_1d)

			model, history = CombinedModel(cnn_model, lstm_model, x_train_2d, y_train_2d_categorical, x_train_1d, x_valid_2d, x_valid_1d, y_valid_2d_categorical, lr, bs, epochs)
